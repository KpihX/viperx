...
A robust project built with ViperX
...
- **Config**: `src/test_config_proj/config.yaml` (Loaded automatically)

- **Environment**: `src/test_config_proj/.env` (Isolated variables)
...
```bash
# Clone the repo
git clone https://github.com/yourname/test-config-proj.git
cd test-config-proj
...
uv run test_config_proj

# Or run a specific script
uv run python src/test_config_proj/main.py
```
...
```python
!pip install git+https://github.com/yourname/test-config-proj.git
```

**Step 2: Use the unified config**
```python
from test_config_proj import get_dataset_path, SETTINGS
