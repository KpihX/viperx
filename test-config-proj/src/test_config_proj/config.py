...
# Load Environment Variables from the isolated .env file in this package (src/<pkg>/.env)
load_dotenv(Path(__file__).parent / ".env")

# Load configuration safely whether installed or local
try:
    # Modern Way (Python 3.9+) - works when installed as a package
    _config_path = importlib.resources.files("test_config_proj").joinpath("config.yaml")
